...
const NATCO_CODE = 'at'
const APP_LANGUAGE = 'de'
const APP_KEY = 'CTnKA63ruKM0JM1doxAXwwyQLLmQiEiy'
const APP_VERSION = '02.0.1080'
const NATCO_KEY = 'NZu7aIg1vFTNLwHcb0Kjhqk54ql9RJj5'
const SITE_URL = 'tv.magenta.at'
  ...
  async parser({ content, channel, date }) {
    ...
    for (let item of items) {
      ...
      programs.push({
        ...
        presenter: parseRoles(detail, ROLE_TYPES.PRESENTER),
...
function parseSeason(item) {
  if (item.season_display_number === 'Folgen') return null // 'Epizode' is 'Episodes' in Croatian
...
function parseRoles(item, roleNames) {
  if (!item.roles) return null;

  // Convert single role name to array for consistent handling
  const rolesToFind = Array.isArray(roleNames) ? roleNames : [roleNames];

  // Filter roles that match any of the provided role names
  return item.roles
    .filter(role => rolesToFind.includes(role.role_name))
    .map(role => ({
